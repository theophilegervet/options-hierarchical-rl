...
class ModelLearning:
    def __init__(self, nfeatures, noptions, discount):
        self.discount = discount
        self.N = np.zeros((nfeatures, noptions))
        self.R = np.zeros((nfeatures, noptions))
        self.P = np.zeros((nfeatures, noptions, nfeatures))

    def start(self, phi, option):
        self.start_phi = phi
        self.option = option
       
    def record(self, phi, cumreward, nsteps):
        self.N[self.start_phi, self.option] += 1
        alpha = (1. / self.N[self.start_phi, self.option])
        self.R[self.start_phi, self.option] += alpha * \
            (cumreward - self.R[self.start_phi, self.option])
        self.P[self.start_phi, self.option, phi] += alpha * (self.discount ** nsteps)
        self.P[self.start_phi, self.option] -= alpha * self.P[self.start_phi, self.option]
